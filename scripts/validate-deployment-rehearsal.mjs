...
for (const file of [
  ...
  'qa/release-room.js',
  'scripts/build-pages-site.mjs',
  'scripts/validate-deployment-rehearsal.mjs',
  'tests/browser/rehearsal.spec.mjs'
]) {
...
const roomDocs = read('docs/RELEASE-ROOM.md');
...
assert(build.includes("createsReleaseApproval: false"), 'deployment authority boundary missing');
...
assert(build.includes("'qa/release-room.html'"), 'Release Room route must be published');
assert(build.includes("'qa/evidence-contract.js'"), 'shared evidence contract must be published');
...
assert(readinessHtml.includes('release-room.html'), 'readiness must link Release Room');
...
assert(contract.includes('window.LarriVerseEvidence'), 'shared evidence contract export missing');
assert(contract.includes('deviceClass === expectedDeviceClass'), 'canonical deviceClass validation missing');
assert(contract.includes('value.environment.maxTouchPoints'), 'canonical touch path missing');
assert(contract.includes("schema: 'larriverse-evidence-bundle'"), 'bundle schema missing');
assert(contract.includes('createsReleaseApproval: false'), 'bundle approval boundary missing');
...
assert(preflightHtml.includes('release-room.html'), 'Release Room link missing');
...
assert(preflightJs.includes('Contract.readEvidenceFile'), 'preflight must use shared evidence parsing');
assert(preflightJs.includes('Contract.validateEvidenceSet'), 'preflight complete-set gate missing');
assert(!preflightJs.includes('deviceRole'), 'preflight must not use obsolete deviceRole');
assert(!preflightJs.includes('value.device?.maxTouchPoints'), 'preflight must not use obsolete touch path');
...
assert(roomHtml.includes('No uploads. No automatic approval. No tag creation.'), 'Release Room boundary missing');
...
assert(roomJs.includes("fetchText('../deployment.json')"), 'Release Room deployment identity request missing');
assert(roomJs.includes("location.protocol === 'https:'"), 'Release Room must require live HTTPS');
assert(roomJs.includes('Contract.createBundle'), 'Release Room bundle creation missing');
assert(roomJs.includes('deploymentReady && evidenceReady()'), 'Release Room complete gate missing');
assert(!roomJs.includes("schema: 'larriverse-release-approval'"), 'Release Room must not create approval');
...
assert(release.deployment?.releaseRoomRoute === 'qa/release-room.html', 'Release Room route mismatch');
...
assert(release.evidenceBundle?.schema === 'larriverse-evidence-bundle', 'bundle release metadata missing');
...
assert(roomDocs.includes('larriverse-evidence-bundle'), 'Release Room docs must name bundle');
assert(roomDocs.includes('does not approve the release'), 'Release Room docs must preserve authority boundary');
...
assert(checklist.includes('Release Room'), 'checklist must include Release Room');

const fakeSha = 'a'.repeat(40);
execFileSync(process.execPath, [path.join(root, 'scripts', 'build-pages-site.mjs')], {
  cwd: root,
  env: { ...process.env, GITHUB_ACTIONS: 'true', GITHUB_SHA: fakeSha, GITHUB_RUN_ID: '12345', GITHUB_RUN_NUMBER: '16', GITHUB_REF_NAME: 'main' },
  stdio: 'pipe'
});
...
assert(deployment.routes.releaseRoom === 'qa/release-room.html', 'built Release Room route mismatch');
assert(exists('_site/qa/readiness.html'), 'built readiness page missing');
assert(exists('_site/qa/evidence-contract.js'), 'built evidence contract missing');
